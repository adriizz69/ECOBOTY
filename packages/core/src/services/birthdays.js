...
import { getBirthdayPremiumPolicy } from "./billing-entitlements.js";

const applyBirthdaySettingsPolicy = (settings, policy) => {
  const safeSettings = settings && typeof settings === "object" ? settings : { ...DEFAULT_SETTINGS };
  const moduleEnabled = policy?.moduleEnabled !== false;
  const roleAnnouncementsEnabled = policy?.roleAnnouncementsEnabled !== false;
  return {
    enabled: moduleEnabled ? Boolean(safeSettings.enabled) : false,
    birthdayRoleId: roleAnnouncementsEnabled ? normalizeRoleId(safeSettings.birthdayRoleId) : "",
    announceChannelId: roleAnnouncementsEnabled ? normalizeChannelId(safeSettings.announceChannelId) : "",
    showAgeInList: toBool(safeSettings.showAgeInList, true)
  };
};
...
export const getOrCreateBirthdaySettings = async (guildId, trx = db) => {
  ...
  const policy = await getBirthdayPremiumPolicy(guildId);
  if (existing) {
    return applyBirthdaySettingsPolicy({
  ...
  return applyBirthdaySettingsPolicy({ ...DEFAULT_SETTINGS }, policy);
};
...
const processGuildRoleSync = async (guildRow) => {
  ...
  const premiumPolicy = await getBirthdayPremiumPolicy(guildDiscordId);
  ...
  if (!premiumPolicy.moduleEnabled || !premiumPolicy.roleAnnouncementsEnabled) {
    const now = new Date();
    const activeAssignments = await db(TABLES.roleAssignments)
      .where({
        guild_id: guildInternalId,
