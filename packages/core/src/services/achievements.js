...
const assertAchievementLimits = async (guildId, { type, excludeId = null } = {}, trx = db) => {
  ...
  if (type === "tier") {
    if (policy.tiersMax !== null && policy.tiersMax !== undefined) {
      await assertGuildLimit({
        guildId,
        limitKey: "achievement_tiers_max",
        currentCount: tierCount,
        context: "achievements.tier_limit"
      });
    } else if (!policy.tiersEnabled) {
      await assertGuildFeatureAccess({
        guildId,
        featureKey: "achievements_tiers",
        context: "achievements.create_tier"
      });
    }
    return;
  }

  if (policy.uniqueMax !== null && policy.uniqueMax !== undefined) {
    await assertGuildLimit({
      guildId,
      limitKey: "achievements_max",
      currentCount: uniqueCount,
      context: "achievements.unique_limit"
    });
  }
};
...
export const createAchievement = async (guildId, input = {}) => {
  return db.transaction(async (trx) => {
    ...
    await assertAchievementLimits(guildId, { type: normalized.type }, trx);
...
export const updateAchievement = async (guildId, achievementId, input = {}) => {
  return db.transaction(async (trx) => {
    ...
    const previousType = existing.type === "tier" ? "tier" : "unique";
    if (previousType !== normalized.type) {
      await assertAchievementLimits(
        guildId,
        { type: normalized.type, excludeId: Number(achievementId) },
        trx
      );
    }
...
export const getAchievementConfigPayload = async (guildId) => {
  ...
  const achievementsWithLocks = await applyRuntimeAchievementLocks(guildId, achievementsWithStats);
  return {
    settings,
    achievements: achievementsWithLocks,
...
const getActiveDefinitionsForEvent = async (guildInternalId, eventKey) => {
  ...
  const guildRow = await db("guilds").where({ id: guildInternalId }).first();
  const guildDiscordId = String(guildRow?.discord_guild_id || "").trim();
  const lockedIds = guildDiscordId
    ? await getRuntimeLockedAchievementIds(guildDiscordId)
    : new Set();
  const now = new Date();
  return definitions.filter((definition) => {
    if (!definition.enabled) return false;
    if (lockedIds.has(Number(definition.id))) return false;
...
export const recordAchievementEvent = async ({
  ...
  const premiumPolicy = await getAchievementsPremiumPolicy(guildId);
  if (!premiumPolicy.moduleEnabled) {
    return { ok: true, skipped: "premium_feature_disabled", processed: 0 };
  }
  const guildInternalId = await getGuildInternalId(guildId, db);
