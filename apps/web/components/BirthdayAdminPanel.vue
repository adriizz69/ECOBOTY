  ...
  <UCard class="card birthday-admin">
        ...
        <article class="sub-card panel-card">
          ...
          <div class="setting-grid">
            <label class="field">
              ...
              <EbSelect v-model="settings.birthdayRoleId" :items="roleSelectItems" />
          ...
          <div class="entry-grid">
            <label ref="memberSearchFieldRef" class="field member-search-field">
              ...
              <div v-if="memberSearchOpen && searchResults.length" class="member-search-results">
                ...
                >
                  <DiscordAvatar
                    v-if="member.avatar || member.id"
                    class="member-result-avatar"
                    :user-id="member.id"
      ...
      <aside class="birthday-side">
        ...
        <article class="sub-card panel-card side-card info-card">
          ...
          <ul class="info-list muted small">
            ...
            <li>Quand un membre quitte le serveur, sa date d'anniversaire est supprimee automatiquement.</li>
          </ul>
        </article>
      </aside>
    </section>

    <article v-if="birthdays.length" class="sub-card panel-card upcoming-birthdays-card">
      <div class="panel-head list-head">
        <div>
          <h4>Prochains anniversaires</h4>
          <p class="muted small">Suivi rapide des dates les plus proches sur le serveur.</p>
        </div>
      </div>
      <div class="upcoming-birthdays-list">
        <article v-for="entry in upcomingBirthdays" :key="`upcoming-${entry.userId}`" class="upcoming-birthday-row">
          <div class="birthday-row-member">
            <div class="member-avatar compact">
              <DiscordAvatar
                :user-id="entry.userId"
                :avatar="memberAvatar(entry.userId)"
                :alt="memberName(entry.userId)"
              />
            </div>
            <div class="member-copy">
              <strong>{{ memberName(entry.userId) }}</strong>
              <span class="muted small">{{ formatDate(entry.birthDate) }}</span>
            </div>
          </div>
          <div class="upcoming-meta">
            <span class="upcoming-countdown">{{ daysLabel(entry.daysUntil) }}</span>
            <span class="muted small">{{ formatDate(entry.nextBirthdayDate) }}</span>
          </div>
        </article>
      </div>
    </article>
    ...
    <article class="sub-card panel-card birthday-list-card">
      ...
      <div v-else class="birthday-list">
        <article v-for="entry in sortedBirthdays" :key="entry.userId" class="birthday-row">
          <div class="birthday-row-member">
            <div class="member-avatar compact">
              <DiscordAvatar
                :user-id="entry.userId"
...
const sortedBirthdays = computed(() =>
  [...birthdays.value].sort((left, right) => {
...
const upcomingBirthdays = computed(() => sortedBirthdays.value.slice(0, 8));
...
.field > :is(input, select, textarea, .eb-select) {
  margin-top: 0;
}
...
.sync-card {
  background:
    ...
    radial-gradient(circle at 12% 12%, rgba(45, 212, 160, 0.16), transparent 55%);
}
