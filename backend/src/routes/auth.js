...
const twitchAuthorizeUrl = ({ guildId, redirect }) => {
  const params = new URLSearchParams({
    ...
    response_type: "code",
    scope: "chat:read chat:edit moderator:read:chatters channel:read:subscriptions",
...
authRouter.get("/discord/login", (req, res) => {
  const redirect = req.query.redirect || process.env.BASE_URL || "http://localhost:3000";
...
authRouter.get("/discord/callback", async (req, res) => {
  ...
  const rawState = req.query.state || process.env.BASE_URL || "http://localhost:3000";
...
authRouter.get("/twitch/callback", async (req, res) => {
  ...
  try {
    ...
    const redirectUrl = redirect || process.env.BASE_URL || "http://localhost:3000";
