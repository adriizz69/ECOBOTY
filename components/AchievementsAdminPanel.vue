<template>
  <UCard class="card achievements-admin">
    ...
    <div v-else class="achievement-sections">
        ...
        <UCard class="editor-modal-card">
          ...
          <div class="editor-layout">
            ...
            <div class="grid">
              ...
              <label>
                Evenement
                <select v-model="form.eventKey">
                  <option v-for="event in availableEvents" :key="event.key" :value="event.key">
                    {{ eventOptionLabel(event.key) }}
                  </option>
                </select>
              </label>
              <label v-if="form.type === 'unique' && form.eventKey === 'role_received'" class="event-condition-field">
                Role a obtenir pour valider le succes
                <select v-model="form.eventTargetRoleId">
              ...
              <div v-for="tier in tierKeys" :key="tier" class="tier-block">
                <div class="tier-head">
                  ...
                  <label class="switch" :title="tier === 'bronze' ? 'Bronze reste actif' : ''">
                    <input v-model="form.tiers[tier].enabled" type="checkbox" :disabled="tier === 'bronze'" />
                    <span class="slider"></span>
                  </label>
                </div>
...
const achievementSections = computed(() => {
  const rows = filteredAchievements.value || [];
        ...
        h("div", { class: "reward-role-inline" }, [
          h(
            "select",
            {
              ...
              onChange: (event) => {
                candidateRef.value = String(event?.target?.value || "");
              }
            },
            [
              h("option", { value: "" }, "Selectionner un role"),
              ...options.map((role) => h("option", { value: String(role.id) }, String(role.name || "Role sans nom")))
            ]
          ),
          h(
            "button",
...
.shape-token {
  ...
  background: linear-gradient(145deg, rgba(191, 219, 254, 0.95), rgba(59, 130, 246, 0.88));
...
.preview-card {
  border: 2px solid var(--ui-border, #7c3aed);
  border-radius: 18px;
